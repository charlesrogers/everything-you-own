...
export function ShelfOrganizer({ shelf, itemCounts, onMoveBin }: ShelfOrganizerProps) {
  ...
  // Build bin placements
  const placements = useMemo(() => allBins.map((bin) => {
    const meta = bin.metadata as Record<string, unknown>
    const colStart = (meta?.col_index as number) ?? 0
    const depthRow = (meta?.depth_row as string) ?? "front"
    const dims = getEffectiveDimensions(bin)
    const widthCols = Math.round(dims.width ?? 11)
    return {
      bin,
      colStart: Math.max(0, Math.min(colStart, gridCols - widthCols)),
      depthRow: depthRow as DepthRow,
      widthCols,
  ...
  const handleDrop = async (col: number, row: "front" | "back") => {
    ...
    const existing = placements.filter((p) =>
      (p.depthRow === row || p.depthRow === "full") && p.colStart === col
    )
  ...
  function renderBinCard(p: typeof placements[number]) {
      ...
      >
        ...
        <div className="flex items-center gap-1.5 mt-0.5">
          <span className="text-[9px] text-muted-foreground">
            {dims.width}&Prime;w
            {p.depthRow !== "full" && <> &middot; {p.depthRow}</>}
          </span>
          {binItems > 0 && (
  ...
  function renderRow(row: "front" | "back") {
    // Include bins with this row OR "full" depth
    const rowBins = placements.filter((p) => p.depthRow === row || p.depthRow === "full")
    ...
    return (
      ...
      >
        ...
        {dragOverSlot && dragOverSlot.row === row && dragBinId && (() => {
          ...
          const dragWidth = dragBin ? Math.round(getEffectiveDimensions(dragBin).width ?? 11) : 11
          return (
            <div
              className="absolute top-1 bottom-1 bg-primary/10 border-2 border-primary/30 rounded pointer-events-none z-0"
              style={{
                left: `${(dragOverSlot.col / gridCols) * 100}%`,
                width: `${(dragWidth / gridCols) * 100}%`,
              }}
            />
          )
        })()}
        ...
        {Array.from({ length: numLogicalCols - 1 }, (_, i) => (
          <div
            key={`guide-${i}`}
            className="absolute top-0 bottom-0 border-l border-dashed border-muted-foreground/10 pointer-events-none z-0"
            style={{ left: `${((i + 1) * typicalBinWidth / shelfWidth) * 100}%` }}
          />
        ))}

        {rowBins.map((p) => renderBinCard(p))}

        {rowBins.length === 0 && (
          <div className="flex items-center justify-center text-[10px] text-muted-foreground/30" style={{ gridColumn: "1 / -1" }}>
  ...
  // Ruler
  const rulerMarks = Array.from({ length: numLogicalCols + 1 }, (_, i) => Math.round(i * (shelfWidth / numLogicalCols)))
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Ruler with column markers */}
      <div className="relative h-5 border-b border-muted-foreground/20">
        {rulerMarks.map((inch, i) => (
          <div
            key={inch}
            className="absolute bottom-0 flex flex-col items-center"
            style={{ left: `${(inch / shelfWidth) * 100}%` }}
          >
            <span className="text-[8px] text-muted-foreground/50">{inch}&Prime;</span>
            <div className="w-px h-2 bg-muted-foreground/20" />
          </div>
        ))}
        {/* Column labels between marks */}
        {Array.from({ length: numLogicalCols }, (_, i) => (
          <div
            key={`label-${i}`}
            className="absolute top-0 text-[9px] text-muted-foreground/40 text-center"
            style={{
              left: `${(i * typicalBinWidth / shelfWidth) * 100}%`,
              width: `${(typicalBinWidth / shelfWidth) * 100}%`,
            }}
          >
            Col {i + 1}
          </div>
